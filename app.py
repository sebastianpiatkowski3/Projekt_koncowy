...
app.config["SECRET_KEY"] = (
    "psnt"  # Klucz prywatny aplikacji dla celów m.in. bezpieczeństwa
)
...
class Profits(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint("file", "instrument", "start_date", "end_date"),
    )
...
def read_data(file):
    ...
    try:
        ...
        flash(
            f"Błąd konwersji danych: {e}. Sprawdź format danych i spróbuj ponownie.",
            "error",
        )
        return None
...
def get_instrument_name(symbol):
    try:
        ...
        info = ticker.info
        instrument_name = info.get(
            "longName", symbol
        )  # Domyślnie ustawia "Nazwa nieznana", jeśli nie ma dostępnej nazwy
...
def apply_conditions(momenty):
    condition = (momenty["crossings_upward"].shift(-1) == True) & (
        momenty["Close"].shift(-1) < momenty["Close"]
    )
...
def count_crossings(df, source="", instrument_name=""):
    ...
    for period1 in periods_1:
        for period2 in periods_2:
            if period1 < period2:
                ...
                crossings_upward, crossings_downward = find_crossings(
                    df_copy, sma_fast, sma_slow
                )
                ...
                df_copy["crossings"] = (
                    df_copy["crossings_upward"] | df_copy["crossings_downward"]
                )
                ...
                if len(df_copy) >= 2:
                    time_difference = (
                        df_copy.iloc[-1]["Datetime"] - df_copy.iloc[0]["Datetime"]
                    )
    ...
    save_data(
        ...
        profit_max,
        len(diffs),
        df_max.iloc[0]["Datetime"],
        df_max.iloc[-1]["Datetime"],
    )
    return (
        results,
        profit_max,
...
def index():
    ...
    if request.form:
        ...
        if "submit_data" in request.form:
            ...
            session["opis"] = (
                f"Dane dla {nazwa_instrumentu} w okresie od {start_date} do {end_date} z interwałem {timeframe}."
            )
...
def charts():
    ...
    if outcome is not None:
        ...
        unique_values_sma_fast = results_df[
            "sma_slow"
        ].unique()  # czyli:   sma_fast_order = ['20' , '30' , '50' , '100']  # Ustal odpowiednią kolejność
        unique_values_sma_slow = results_df["sma_fast"].unique()
        results_df["sma_slow"] = pd.Categorical(
            results_df["sma_slow"], categories=unique_values_sma_fast, ordered=True
        )
        results_df["sma_fast"] = pd.Categorical(
            results_df["sma_fast"], categories=unique_values_sma_slow, ordered=True
        )
    ...
    W powyższym kodzie, results_df['sma_fast'].astype(str) i results_df['sma_slow'].astype(str) przekształcają ‘sma_fast’ i ‘sma_slow’ na łańcuchy znaków. Dzięki temu, każda unikalna wartość ‘sma_fast’ i ‘sma_slow’ jest traktowana jako osobna kategoria, a nie jako liczba, co powinno sprawić, że wszystkie komórki na mapie ciepła będą miały tę samą wielkość."""
        ...
        fig = px.imshow(
            results_df.pivot(
                index="sma_slow", columns="sma_fast", values="daily_profit"
            ),  # Dane
            labels=dict(
                x="Okresy sredniej szybkiej", y="Okresy sredniej wolnej", color="Profit"
            ),
            text_auto=True,  # Automatyczne wyświetlanie wartości,
            width=570,
            height=570,  # Szerokość i wysokość wykresu
            color_continuous_scale="blues",
        )  # Wybierz inną paletę kolorów, jeśli chcesz
        fig.update_layout(
        ...
        )  # Ustawienie koloru tła obszaru z wykresem
        ...
        fig.update_layout(
            title="Średnie dla największego profitu.  SMA Fast: "
            + str(sm1_max)
            + ", SMA Slow:  "
            + str(sm2_max),
            font=dict(
        ...
        fig.update_layout(
            title="Średnie dla najmniejszego profitu.  SMA Fast: "
            + str(sm1_min)
            + ", SMA Slow:  "
            + str(sm2_min),
            font=dict(
