...
df = pd.read_csv('DAX40_5pips.csv', sep=',', header=None)
...
for index, row in df.iterrows():
    distance2 = 0
    # Iterujemy przez każdy wiersz
    for wiersz in range(index+1, zakres): #  TODO: for wiersz in range(index+1, len(df)):
        distance2 += 1
        if candle[wiersz] == 1:
            profit += 1
        ...
        if profit <= -stop_loss:
            ...
            #  exit for loop
            break
        if wiersz == zakres - 1: # TODOif wiersz == len(df)-1:
